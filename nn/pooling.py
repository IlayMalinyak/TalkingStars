...
class AttentionBasedPooling(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(AttentionBasedPooling, self).__init__()
        self.gmt = GraphMultisetTransformer(
            channels=in_channels,  # Not 'in_channels' but 'channels'
            k=out_channels,
            heads=8
        )

    def forward(self, batched_input):
        batch_size, num_nodes, num_features = batched_input.shape

        # 1. Reshape to [batch_size * num_nodes, num_features]
        reshaped_input = batched_input.reshape(-1, num_features)
        ...
        # 4. Apply GMT
        pooled_features = self.gmt(reshaped_input, batch_index)
